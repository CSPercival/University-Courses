...
pair<vector<pair<int,int>>,string> generate_random_moves(vector<pair<int,int>>* states,int numof_moves,int sizeof_S,int numof_attempts){
    // string new_path="";
    // int direction;
    ...
    int tmp_sizeof_S=1;
    do{
        if(ctr1==numof_attempts){
            ctr1=0;
            tmp_sizeof_S=min(tmp_sizeof_S+1,sizeof_S);
        }
        tmp_moves=numof_moves;
        new_states=(*states);
        new_path="";
        ctr1++;
        ctr2++;
        do{
            do{
                direction=random_number(0,4);
            } while(!new_path.empty() && (new_path.back() == rev_sxy[direction]));
            new_states=generate_move(&new_states,direction);
            new_path+=sxy[direction];
            tmp_moves--;
        } while(new_states.size()>tmp_sizeof_S && tmp_moves>0);
    } while(new_states.size()>tmp_sizeof_S);
    //cout << "FOUND SOLUTION IN " << ctr2 << endl; 
    return {new_states,new_path};
}
...
int main(){
    ...
    while(ans_path==PATH_NOT_FOUND){
        //cout << ctr1++ << endl;
        part_ans=generate_random_moves(&S,110,4,1000);
        //cout << "FOUND IT-----------------------------------------\n";
