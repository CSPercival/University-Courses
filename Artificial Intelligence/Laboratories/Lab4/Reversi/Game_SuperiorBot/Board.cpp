...
class Board{
    ...
    int evaluate(int player){
        ...
        //int respl = 0,resopp = 0;
        vector<int> res(3,0);
        // vector<int> directions;
        // for(int i = 0; i < 4; i++){
        //     if(get_val(corners[i]) != player) continue;
        //     respl += dfs(corners[i],(&vis),player,cover_dir[i]);
        // }

        // for(auto corner : corners){
        //     if(get_val(corner) != opponent) continue;
        //     resopp += dfs(corner,(&vis),opponent);
        // }
        int limit;
        for(int plr = 1; plr <=2; plr++){
            ...
            for(int row = 0; row < MAX_N; row++){
                ...
                for(column = 0; column < limit; column++){
                    ...
                    vis[row][column] = plr;
                    res[plr] += stable_disc_weight - weights[row][column];
                }
                limit = column;
            }
            limit = -1;
