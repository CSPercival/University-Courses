...
class Game{
    ...
    void setup(){
        ...
        ans = player1->perform(ONEMORE_MESS,emptyPP);
        if(ans.st != RDY_MESS){
            cerr << "PLAYER 1 IS NOT READY\n";
        }

        ans = player2->perform(ONEMORE_MESS,emptyPP);
        if(ans.st != RDY_MESS){
    ...
    int play(){
        ...
        answer = player1->perform(UGO_MESS,emptyPP);

        /* ER */ cerr << "P1 -> G: " << answer.st << " ";
        // for(auto arg : answer.nd) cerr << arg << " ";
        /* ER */ cerr << answer.nd.st.st << " " << answer.nd.st.nd << " " << answer.nd.nd.st << " "  << answer.nd.nd.nd;
        /* ER */ cerr << "\n";

        if(answer.st == IDO_MESS){
            //move = {answer.nd[0],answer.nd[1]};
            if(board.correct_move(answer.nd.st,answer.nd.nd,turn)){
                board.make_move(answer.nd.st,answer.nd.nd,turn);
            } else {
